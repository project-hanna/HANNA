...
LOGO_FILE = "logo1.png"
def get_base64_logo(p):
    if os.path.exists(p):
        with open(p, "rb") as f: return base64.b64encode(f.read()).decode()
    return ""
LOGO_B64 = get_base64_logo(LOGO_FILE)

# --- 2. CSS BDD9 (STABLE & CONCIS) ---
st.markdown(f"""
    <style>
    .main .block-container {{ max-width: 550px !important; padding: 4rem 1rem !important; margin: 0 auto !important; display: flex !important; flex-direction: column !important; align-items: center !important; }}
    [data-testid="stVerticalBlock"] {{ align-items: center !important; width: 100% !important; }}
    .hanna-header {{ width: 100% !important; margin-bottom: 3.5rem !important; text-align: center !important; }}
    .hanna-logo {{ width: 120px !important; margin-bottom: 25px !important; }}
    .hanna-title {{ font-family: 'Inter', sans-serif; font-weight: 200; font-size: 52px; letter-spacing: 14px; margin-right: -14px !important; text-transform: uppercase; line-height: 1; }}
    div.stTextInput {{ width: 100% !important; max-width: 480px !important; }}
    div.stTextInput input {{ text-align: left !important; padding: 0 20px !important; border-radius: 12px !important; border: 1px solid #EEE !important; height: 50px !important; }}
    div.stTextInput input::placeholder, div.stTextInput input::-webkit-input-placeholder {{ text-align: center !important; }}
...
# --- 3. ARCHITECTURE CSS BDD9 (STABLE) ---
st.markdown(f"""
    <style>
...
for note in st.session_state.notes:
    st.markdown(f"""
        <div style="padding: 15px; border-radius: 12px; background: #FAFAFA; border: 1px solid #F0F0F0; margin-bottom: 12px; width: 100%; text-align: left;">
            <small style="color: #007BFF; font-weight: 800; font-size: 11px;">{note['time
